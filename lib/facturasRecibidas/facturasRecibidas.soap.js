...
var facturasRecibidasSoapAPI = {
    sendXml: function (jsObject, done) {
        ...
        soap.createClient(urlWsdl, { wsdl_options: { securityLevel: 0 } }, function (err, client) {
            if (err) return done(err);
            client.wsdl.definitions.xmlns.ns1 = urlXsd;
            client.wsdl.xmlnsInEnvelope = client.wsdl._xmlnsMap()
            client.setSecurity(new soap.ClientSSLSecurityPFX(pfxBuffer, cert.pass, { securityLevel: 0, rejectUnauthorized: false, minVersion: 'TLSv1' }));
            if (tipo == "PROD") {
